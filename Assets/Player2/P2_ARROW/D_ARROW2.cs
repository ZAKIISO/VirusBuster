...
public class D_ARROW2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.position = new Vector3(Ppos2.x, Ppos2.y - 1, -2.0f);    //下矢印を常にPlayerの下隣に配置
    }
    ...
    void OnMouseUpAsButton()
    {
        if (Rscript.rouletteNO > 0 && Rscript.Pstart == true)//ルーレットで出た数字が0より大きければ＆プレイヤーの操作が開放されていたら
        {
            if (Ctrigger == true)
            {
                Player2.transform.position = new Vector3(Ppos2.x, Ppos2.y - 1, Ppos2.z);//PlayerのY座標を-1移動
                Pscript2.direction2 = 3;                                                //プレイヤーの向いている方向を下
